...
<template>
    <Teleport to="body" v-if="dragging === piece.square">
      <div
        ...
        :style="{ left: dragging === piece.square ? `${pieceLeft}px` : `30px`, top: dragging === piece.square ? `${pieceTop}px` : `0px`, width: `${size}px`, height: `${size}px` }"
      ></div>
    </Teleport>
    <div 
      v-else
      :class="`piece ${piece.color}${piece.type}`"
      :style="{ transform: `translateX(${(piece.file - 1) * size}px) translateY(${(piece.rank - 1) * size}px)`, width: `${size}px`, height: `${size}px` }"
    ></div>
</template>